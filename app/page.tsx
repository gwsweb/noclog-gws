...
function ifaceColor(iface?: string): string {
  if (!iface) return "text-[#3a5068]";
  return IFACE_COLORS[iface] ?? "text-[#5a8aaa]";
}
...
export default function NOCDashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, errors: 0, success: 0, disconn: 0, eth7: 0, vlan: 0 });
  ...
  const updateStats = useCallback((entries: LogEntry[]) => {
    setStats({
      ...
      eth7:    entries.filter(e => e.iface === "ETH7").length,
      vlan:    entries.filter(e => e.iface?.startsWith("VLAN2027")).length,
    });
  }, []);
  ...
  const clearLogs = () => {
    ...
    setStats({ total: 0, errors: 0, success: 0, disconn: 0, eth7: 0, vlan: 0 });
  };

  const filteredLogs = logs.filter(log => {
    const matchLevel  = levelFilter === "all" || log.level === levelFilter;
    const matchIface  = ifaceFilter === "all" || log.iface === ifaceFilter;
    const matchSearch = search === "" ||
      log.message.toLowerCase().includes(search.toLowerCase()) ||
      (log.user?.toLowerCase().includes(search.toLowerCase()) ?? false) ||
      (log.ip?.includes(search) ?? false);
    return matchLevel && matchIface && matchSearch;
  });
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0a0c0f] text-[#c8d4e0] font-sans overflow-hidden">
      ...
      <header className="flex-none border-b border-[#1e2530] bg-[#0f1216]">
        <div className="flex items-center justify-between px-4 py-2">
          <div className="flex flex-col">
            <span className="text-xs font-semibold tracking-[0.2em] text-[#4a8abf] uppercase">NOC Portal</span>
            <span className="text-[10px] text-[#4a5a6a] tracking-widest font-mono uppercase">
              MikroTik PPPoE · ETH7 &amp; VLAN2027
            </span>
          </div>
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-t border-[#1e2530] flex-wrap">
          ...
          <div className="flex gap-1">
            {(["all","error","success","disconnect","warning"] as const).map(lvl => (
              <button key={lvl} onClick={() => setLevelFilter(lvl)}
                className={`text-[10px] font-mono px-2 py-1 rounded border transition-colors uppercase tracking-wider ${
                  levelFilter === lvl
                    ...
                    : "bg-transparent border-[#1e2530] text-[#4a5a6a] hover:border-[#2a3545]"
                }`}>
          ...
          {/* Port filter — 3 big buttons */}
          <div className="flex gap-1">
            {[
              { key: "all",       label: "ALL PORTS",  cls: "text-[#4a8abf] border-[#3b6088]"  },
              { key: "ETH7",      label: "ETH7",       cls: "text-yellow-400 border-yellow-700" },
              { key: "VLAN2027-1",label: "VLAN2027-1", cls: "text-orange-400 border-orange-700" },
            ].map(({ key, label, cls }) => (
              <button key={key} onClick={() => setIfaceFilter(key)}
                className={`text-[10px] font-mono px-3 py-1 rounded border transition-colors tracking-wider font-semibold ${
                  ifaceFilter === key
                    ? `${cls} bg-white/5`
                    : "border-[#1e2530] text-[#4a5a6a] hover:border-[#2a3545]"
                }`}>
                {label}
              </button>
            ))}
      ...
      <div className="flex-none flex items-center px-4 py-1 bg-[#0d1017] border-b border-[#1e2530] text-[10px] font-mono text-[#2a3545] uppercase tracking-widest">
        ...
        <span className="w-[78px]">Level</span>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 text-[#2a3545]">
            ...
            <div className="text-center">
              ...
              <p className="text-[11px] mt-1 text-[#1e2530]">
                {search || levelFilter !== "all" || ifaceFilter !== "all"
                  ? "No results match your filter"
                  : "Waiting for ETH7 / VLAN2027 PPPoE events..."}
              </p>
            </div>
          </div>
        ) : (
...
function LogRow({ log, isNew }: { log: LogEntry; isNew: boolean }) {
  return (
    <div className={`flex items-center px-4 py-[3px] text-[11px] font-mono hover:bg-white/[0.02] transition-colors cursor-default ${LEVEL_CLASS[log.level]} ${isNew ? "log-new" : ""}`}>
      ...
      <span className="w-[110px] shrink-0 text-[#3a5a7a] hidden lg:block">{log.ip ?? "—"}</span>
